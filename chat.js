...
window.onload = () => {
  updateChatList();
  updateLongTermChatList();
  populateModels();
  adjustPadding();
  autoFocusInput();

  document.getElementById("delete-chat").addEventListener("click", deleteShorterChat);
  document.getElementById("delete-chat1").addEventListener("click", deleteChat);
  document.getElementById("saveName1").addEventListener("click", saveLongTermChat);
  document.getElementById("saveName").addEventListener("click", saveChat);
  document.getElementById("chat-select").addEventListener("change", loadSelectedLongChat);
  document.getElementById("chat-select-short").addEventListener("change", loadSelectedChat);
  document.getElementById("host-address").addEventListener("change", setHostAddress);
}

function deleteShorterChat() {
  const selectedChat = document.getElementById("chat-select-short").value;
...
// Function to save chat long term with a unique name
function saveLongTermChat() {

  const chatName = document.getElementById('userName1').value;

  // Close the modal
  const bootstrapModal = bootstrap.Modal.getInstance(document.getElementById('nameModal1'));
  bootstrapModal.hide();
...
function saveChat() {
  ...
  const context = document.getElementById('chat-history').context;
  const model = getSelectedModel();
  sessionStorage.setItem(chatName, JSON.stringify({"history":history, "context":context, "model": model}));
  updateChatList();
}
...
function loadSelectedChat() {
  const selectedChat = document.getElementById("chat-select-short").value;
  const obj = JSON.parse(sessionStorage.getItem(selectedChat));
...
function updateChatList() {
  const chatList = document.getElementById("chat-select-short");
...
function updateLongTermChatList() {
